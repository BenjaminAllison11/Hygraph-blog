...
const FeaturedPostCard = ({ post }) => (
  <div className="relative h-72 rounded-lg">
    ...
    <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full rounded-full">
      <p className="text-white mb-4 text-shadow font-bold text-xs">{moment(post.createdAt).format('MMM DD, YYYY')}</p>
      <p className="text-white mb-4 text-shadow font-bold text-2xl text-center">{post.title}</p>
      <div className="flex items-center absolute bottom-5 w-full justify-center">
        ...
        />
        <p className="inline align-middle text-white text-shadow ml-2 font-bold">{post.author.name}</p>
      </div>
    </div>
    <Link href={`/post/${post.slug}`}><span className="cursor-pointer absolute w-full h-full rounded-full" /></Link>
  </div>
);
